...
function HeaderMap.Start:SOURCE (fsm, token)
    ...
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::Start.SOURCE(token)\n")
    end
    fsm:clearState()
    ctxt:setSource(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::Start.SOURCE(token)\n")
    end
...
function HeaderMap.Start:CLASS_NAME (fsm, token)
    ...
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::Start.CLASS_NAME(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::Start.CLASS_NAME(token)\n")
    end
...
function HeaderMap.Start:START_STATE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::Start.START_STATE(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::Start.START_STATE(token)\n")
    end
...
function HeaderMap.Start:FSM_CLASS_NAME (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::Start.FSM_CLASS_NAME(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::Start.FSM_CLASS_NAME(token)\n")
    end
...
function HeaderMap.Start:HEADER_FILE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::Start.HEADER_FILE(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::Start.HEADER_FILE(token)\n")
    end
...
function HeaderMap.Start:INCLUDE_FILE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::Start.INCLUDE_FILE(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::Start.INCLUDE_FILE(token)\n")
    end
...
function HeaderMap.Start:PACKAGE_NAME (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::Start.PACKAGE_NAME(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::Start.PACKAGE_NAME(token)\n")
    end
...
function HeaderMap.Start:IMPORT (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::Start.IMPORT(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::Start.IMPORT(token)\n")
    end
...
function HeaderMap.Start:DECLARE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::Start.DECLARE(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::Start.DECLARE(token)\n")
    end
...
function HeaderMap.Start:ACCESS (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::Start.ACCESS(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::Start.ACCESS(token)\n")
    end
...
function HeaderMap.Start:MAP_NAME (fsm, token)
    ...
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::Start.MAP_NAME(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::Start.MAP_NAME(token)\n")
    end
...
function HeaderMap.Context:WORD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::Context.WORD(token)\n")
    end
    fsm:clearState()
    ctxt:setContext(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::Context.WORD(token)\n")
    end
...
function HeaderMap.StartState:WORD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::StartState.WORD(token)\n")
    end
    fsm:clearState()
    ctxt:setStartState(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::StartState.WORD(token)\n")
    end
    fsm:setState(HeaderMap.Start)
    fsm:getState():Entry(fsm)
end
...
function HeaderMap.HeaderFile:SOURCE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::HeaderFile.SOURCE(token)\n")
    end
    fsm:clearState()
    ctxt:setHeader(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::HeaderFile.SOURCE(token)\n")
    end
    fsm:setState(HeaderMap.Start)
    fsm:getState():Entry(fsm)
end
...
function HeaderMap.IncludeFile:SOURCE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::IncludeFile.SOURCE(token)\n")
    end
    fsm:clearState()
    ctxt:addInclude(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::IncludeFile.SOURCE(token)\n")
    end
    fsm:setState(HeaderMap.Start)
    fsm:getState():Entry(fsm)
end
...
function HeaderMap.Package:WORD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::Package.WORD(token)\n")
    end
    fsm:clearState()
    ctxt:setPackageName(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::Package.WORD(token)\n")
    end
    fsm:setState(HeaderMap.Start)
    fsm:getState():Entry(fsm)
end
...
function HeaderMap.FsmClassName:WORD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::FsmClassName.WORD(token)\n")
    end
    fsm:clearState()
    ctxt:setFsmClassName(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::FsmClassName.WORD(token)\n")
    end
    fsm:setState(HeaderMap.Start)
    fsm:getState():Entry(fsm)
end
...
function HeaderMap.Import:SOURCE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::Import.SOURCE(token)\n")
    end
    fsm:clearState()
    ctxt:addImport(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::Import.SOURCE(token)\n")
    end
    fsm:setState(HeaderMap.Start)
    fsm:getState():Entry(fsm)
end
...
function HeaderMap.Declare:SOURCE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::Declare.SOURCE(token)\n")
    end
    fsm:clearState()
    ctxt:addDeclare(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::Declare.SOURCE(token)\n")
    end
    fsm:setState(HeaderMap.Start)
    fsm:getState():Entry(fsm)
end
...
function HeaderMap.Access:SOURCE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::Access.SOURCE(token)\n")
    end
    fsm:clearState()
    ctxt:setAccessLevel(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::Access.SOURCE(token)\n")
    end
    fsm:setState(HeaderMap.Start)
    fsm:getState():Entry(fsm)
end
...
function HeaderMap.StartError:SOURCE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::StartError.SOURCE(token)\n")
    end
    fsm:clearState()
    ctxt:setSource(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::StartError.SOURCE(token)\n")
    end
    fsm:setState(HeaderMap.Start)
    fsm:getState():Entry(fsm)
end
...
function HeaderMap.StartError:CLASS_NAME (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::StartError.CLASS_NAME(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::StartError.CLASS_NAME(token)\n")
    end
    fsm:setState(HeaderMap.Context)
    fsm:getState():Entry(fsm)
end
...
function HeaderMap.StartError:START_STATE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::StartError.START_STATE(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::StartError.START_STATE(token)\n")
    end
    fsm:setState(HeaderMap.StartState)
    fsm:getState():Entry(fsm)
end
...
function HeaderMap.StartError:FSM_CLASS_NAME (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::StartError.FSM_CLASS_NAME(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::StartError.FSM_CLASS_NAME(token)\n")
    end
    fsm:setState(HeaderMap.FsmClassName)
    fsm:getState():Entry(fsm)
end
...
function HeaderMap.StartError:HEADER_FILE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::StartError.HEADER_FILE(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::StartError.HEADER_FILE(token)\n")
    end
    fsm:setState(HeaderMap.HeaderFile)
    fsm:getState():Entry(fsm)
end
...
function HeaderMap.StartError:INCLUDE_FILE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::StartError.INCLUDE_FILE(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::StartError.INCLUDE_FILE(token)\n")
    end
    fsm:setState(HeaderMap.IncludeFile)
    fsm:getState():Entry(fsm)
end
...
function HeaderMap.StartError:PACKAGE_NAME (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::StartError.PACKAGE_NAME(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::StartError.PACKAGE_NAME(token)\n")
    end
    fsm:setState(HeaderMap.Package)
    fsm:getState():Entry(fsm)
end
...
function HeaderMap.StartError:IMPORT (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::StartError.IMPORT(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::StartError.IMPORT(token)\n")
    end
    fsm:setState(HeaderMap.Import)
    fsm:getState():Entry(fsm)
end
...
function HeaderMap.StartError:DECLARE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::StartError.DECLARE(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::StartError.DECLARE(token)\n")
    end
    fsm:setState(HeaderMap.Declare)
    fsm:getState():Entry(fsm)
end
...
function HeaderMap.StartError:ACCESS (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::StartError.ACCESS(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::StartError.ACCESS(token)\n")
    end
    fsm:setState(HeaderMap.Access)
    fsm:getState():Entry(fsm)
end
...
function HeaderMap.StartError:MAP_NAME (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: HeaderMap::StartError.MAP_NAME(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : HeaderMap::StartError.MAP_NAME(token)\n")
    end
    fsm:setState(HeaderMap.Start)
    fsm:getState():Entry(fsm)
    fsm:pushState(MapsMap.MapName)
...
function MapsMap.MapStart:MAP_NAME (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::MapStart.MAP_NAME(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::MapStart.MAP_NAME(token)\n")
    end
...
function MapsMap.MapStartError:MAP_NAME (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::MapStartError.MAP_NAME(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::MapStartError.MAP_NAME(token)\n")
    end
    fsm:setState(MapsMap.MapName)
    fsm:getState():Entry(fsm)
end
...
function MapsMap.MapName:WORD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::MapName.WORD(token)\n")
    end
    fsm:clearState()
    ctxt:createMap(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::MapName.WORD(token)\n")
    end
...
function MapsMap.MapName:EOD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::MapName.EOD(token)\n")
    end
    fsm:clearState()
    ctxt:_error("Name expected after \"%map\".")
    ctxt:createMap("no_named")
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::MapName.EOD(token)\n")
    end
...
function MapsMap.MapStates:EOD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::MapStates.EOD(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::MapStates.EOD(token)\n")
    end
    fsm:setState(MapsMap.States)
    fsm:getState():Entry(fsm)
end
...
function MapsMap.MapStates:WORD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::MapStates.WORD(token)\n")
    end
    fsm:clearState()
    ctxt:_error("Expecting %% after \"%map mapname\".")
    ctxt:createState(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::MapStates.WORD(token)\n")
    end
...
function MapsMap.MapStatesError:EOD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::MapStatesError.EOD(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::MapStatesError.EOD(token)\n")
    end
    fsm:setState(MapsMap.States)
    fsm:getState():Entry(fsm)
end
...
function MapsMap.MapStatesError:MAP_NAME (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::MapStatesError.MAP_NAME(token)\n")
    end
    fsm:clearState()
    ctxt:addMap()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::MapStatesError.MAP_NAME(token)\n")
    end
    fsm:setState(MapsMap.MapName)
    fsm:getState():Entry(fsm)
end
...
function MapsMap.MapStatesError:WORD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::MapStatesError.WORD(token)\n")
    end
    fsm:clearState()
    ctxt:createState(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::MapStatesError.WORD(token)\n")
    end
    fsm:setState(MapsMap.StateStart)
    fsm:getState():Entry(fsm)
end
...
function MapsMap.States:EOD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::States.EOD(token)\n")
    end
    fsm:clearState()
    ctxt:addMap()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::States.EOD(token)\n")
    end
...
function MapsMap.States:WORD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::States.WORD(token)\n")
    end
    fsm:clearState()
    ctxt:createState(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::States.WORD(token)\n")
    end
    fsm:setState(MapsMap.StateStart)
    fsm:getState():Entry(fsm)
end
...
function MapsMap.States:ENTRY (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::States.ENTRY(token)\n")
    end
    fsm:clearState()
    ctxt:_error("Expecting either a new state definition or end of map (%%).")
    ctxt:createState(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::States.ENTRY(token)\n")
    end
...
function MapsMap.States:EXIT (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::States.EXIT(token)\n")
    end
    fsm:clearState()
    ctxt:_error("Expecting either a new state definition or end of map (%%).")
    ctxt:createState(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::States.EXIT(token)\n")
    end
...
function MapsMap.States:LEFT_BRACE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::States.LEFT_BRACE(token)\n")
    end
    fsm:clearState()
    ctxt:_error("Expecting either a new state definition or end of map (%%).")
    ctxt:createState(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::States.LEFT_BRACE(token)\n")
    end
...
function MapsMap.States:MAP_NAME (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::States.MAP_NAME(token)\n")
    end
    fsm:clearState()
    ctxt:_error("Expecting \"%%\" before another \"%map\".")
    ctxt:addMap()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::States.MAP_NAME(token)\n")
    end
    fsm:setState(MapsMap.MapName)
    fsm:getState():Entry(fsm)
end
...
function MapsMap.StateStart:ENTRY (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::StateStart.ENTRY(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::StateStart.ENTRY(token)\n")
    end
    fsm:setState(MapsMap.EntryStart)
    fsm:getState():Entry(fsm)
end
...
function MapsMap.StateStart:EXIT (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::StateStart.EXIT(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::StateStart.EXIT(token)\n")
    end
    fsm:setState(MapsMap.ExitStart)
    fsm:getState():Entry(fsm)
end
...
function MapsMap.StateStart:LEFT_BRACE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::StateStart.LEFT_BRACE(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::StateStart.LEFT_BRACE(token)\n")
    end
    fsm:setState(MapsMap.TransEnd)
    fsm:getState():Entry(fsm)
    fsm:pushState(TransitionsMap.Start)
...
function MapsMap.StateStartError:ENTRY (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::StateStartError.ENTRY(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::StateStartError.ENTRY(token)\n")
    end
    fsm:setState(MapsMap.EntryStart)
    fsm:getState():Entry(fsm)
end
...
function MapsMap.StateStartError:EXIT (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::StateStartError.EXIT(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::StateStartError.EXIT(token)\n")
    end
    fsm:setState(MapsMap.ExitStart)
    fsm:getState():Entry(fsm)
end
...
function MapsMap.StateStartError:LEFT_BRACE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::StateStartError.LEFT_BRACE(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::StateStartError.LEFT_BRACE(token)\n")
    end
    fsm:setState(MapsMap.TransEnd)
    fsm:getState():Entry(fsm)
    fsm:pushState(TransitionsMap.Start)
...
function MapsMap.EntryStart:LEFT_BRACE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::EntryStart.LEFT_BRACE(token)\n")
    end
    fsm:clearState()
    ctxt:createActionList()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::EntryStart.LEFT_BRACE(token)\n")
    end
...
function MapsMap.ExitStart:LEFT_BRACE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: MapsMap::ExitStart.LEFT_BRACE(token)\n")
    end
    fsm:clearState()
    ctxt:createActionList()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : MapsMap::ExitStart.LEFT_BRACE(token)\n")
    end
...
function TransitionsMap.Start:RIGHT_BRACE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::Start.RIGHT_BRACE(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::Start.RIGHT_BRACE(token)\n")
    end
    fsm:popState()
    fsm:transitionsDone()
end
...
function TransitionsMap.Start:WORD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::Start.WORD(token)\n")
    end
    fsm:clearState()
    ctxt:storeTransitionName(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::Start.WORD(token)\n")
    end
...
function TransitionsMap.TransError:RIGHT_BRACE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransError.RIGHT_BRACE(token)\n")
    end
    fsm:clearState()
    ctxt:addState()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransError.RIGHT_BRACE(token)\n")
    end
    fsm:popState()
    fsm:transitionsDone()
end
...
function TransitionsMap.TransError:WORD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransError.WORD(token)\n")
    end
    fsm:clearState()
    ctxt:storeTransitionName(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransError.WORD(token)\n")
    end
    fsm:setState(TransitionsMap.TransStart)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.TransStart:LEFT_PAREN (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransStart.LEFT_PAREN(token)\n")
    end
    fsm:clearState()
    ctxt:createParamList()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransStart.LEFT_PAREN(token)\n")
    end
...
function TransitionsMap.TransStart:LEFT_BRACKET (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransStart.LEFT_BRACKET(token)\n")
    end
    fsm:clearState()
    ctxt:createTransition()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransStart.LEFT_BRACKET(token)\n")
    end
...
function TransitionsMap.TransStart:PUSH (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransStart.PUSH(token)\n")
    end
    fsm:clearState()
    ctxt:createTransition()
    ctxt:createGuard()
    ...
    ctxt:setEndState()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransStart.PUSH(token)\n")
    end
...
function TransitionsMap.TransStart:POP (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransStart.POP(token)\n")
    end
    fsm:clearState()
    ctxt:createTransition()
    ctxt:createGuard()
    ...
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransStart.POP(token)\n")
    end
...
function TransitionsMap.TransStart:JUMP (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransStart.JUMP(token)\n")
    end
    fsm:clearState()
    ctxt:createTransition()
    ctxt:createGuard()
    ...
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransStart.JUMP(token)\n")
    end
...
function TransitionsMap.TransStart:WORD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransStart.WORD(token)\n")
    end
    fsm:clearState()
    ctxt:createTransition()
    ctxt:createGuard()
    ...
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransStart.WORD(token)\n")
    end
...
function TransitionsMap.TransStartError:LEFT_PAREN (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransStartError.LEFT_PAREN(token)\n")
    end
    fsm:clearState()
    ctxt:createParamList()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransStartError.LEFT_PAREN(token)\n")
    end
    fsm:setState(TransitionsMap.TransParams)
    fsm:getState():Entry(fsm)
    fsm:pushState(ParamsMap.Start)
...
function TransitionsMap.TransStartError:LEFT_BRACKET (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransStartError.LEFT_BRACKET(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransStartError.LEFT_BRACKET(token)\n")
    end
    fsm:setState(TransitionsMap.TransGuard)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.TransStartError:LEFT_BRACE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransStartError.LEFT_BRACE(token)\n")
    end
    fsm:clearState()
    ctxt:createGuard()
    ctxt:setTransType("TRANS_SET")
    ...
    ctxt:createActionList()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransStartError.LEFT_BRACE(token)\n")
    end
...
function TransitionsMap.TransStartError:PUSH (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransStartError.PUSH(token)\n")
    end
    fsm:clearState()
    ctxt:createGuard()
    ctxt:setTransType("TRANS_PUSH")
    ctxt:setEndState()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransStartError.PUSH(token)\n")
    end
    fsm:setState(TransitionsMap.PushStart)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.TransStartError:POP (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransStartError.POP(token)\n")
    end
    fsm:clearState()
    ctxt:createGuard()
    ctxt:setTransType("TRANS_POP")
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransStartError.POP(token)\n")
    end
    fsm:setState(TransitionsMap.PopStart)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.TransStartError:JUMP (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransStartError.JUMP(token)\n")
    end
    fsm:clearState()
    ctxt:createGuard()
    ctxt:setTransType("TRANS_SET")
    ctxt:setEndState()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransStartError.JUMP(token)\n")
    end
    fsm:setState(TransitionsMap.JumpStart)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.TransStartError:RIGHT_BRACE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransStartError.RIGHT_BRACE(token)\n")
    end
    fsm:clearState()
    ctxt:addGuard()
    ctxt:addTransition()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransStartError.RIGHT_BRACE(token)\n")
    end
...
function TransitionsMap.TransNext:LEFT_BRACKET (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransNext.LEFT_BRACKET(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransNext.LEFT_BRACKET(token)\n")
    end
    fsm:setState(TransitionsMap.TransGuard)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.TransNext:PUSH (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransNext.PUSH(token)\n")
    end
    fsm:clearState()
    ctxt:createGuard()
    ctxt:setTransType("TRANS_PUSH")
    ctxt:setEndState()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransNext.PUSH(token)\n")
    end
    fsm:setState(TransitionsMap.PushStart)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.TransNext:POP (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransNext.POP(token)\n")
    end
    fsm:clearState()
    ctxt:createGuard()
    ctxt:setTransType("TRANS_POP")
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransNext.POP(token)\n")
    end
    fsm:setState(TransitionsMap.PopStart)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.TransNext:JUMP (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransNext.JUMP(token)\n")
    end
    fsm:clearState()
    ctxt:createGuard()
    ctxt:setTransType("TRANS_SET")
    ctxt:setEndState()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransNext.JUMP(token)\n")
    end
    fsm:setState(TransitionsMap.JumpStart)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.TransNext:WORD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransNext.WORD(token)\n")
    end
    fsm:clearState()
    ctxt:createGuard()
    ctxt:setTransType("TRANS_SET")
    ctxt:setEndState(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransNext.WORD(token)\n")
    end
    fsm:setState(TransitionsMap.SimpleTrans)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.TransNextError:LEFT_PAREN (fsm, token)
    ...
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransNextError.LEFT_PAREN(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransNextError.LEFT_PAREN(token)\n")
    end
end
...
function TransitionsMap.TransNextError:LEFT_BRACKET (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransNextError.LEFT_BRACKET(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransNextError.LEFT_BRACKET(token)\n")
    end
    fsm:setState(TransitionsMap.TransGuard)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.TransNextError:PUSH (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransNextError.PUSH(token)\n")
    end
    fsm:clearState()
    ctxt:createGuard()
    ctxt:setTransType("TRANS_PUSH")
    ctxt:setEndState()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransNextError.PUSH(token)\n")
    end
    fsm:setState(TransitionsMap.PushStart)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.TransNextError:POP (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransNextError.POP(token)\n")
    end
    fsm:clearState()
    ctxt:createGuard()
    ctxt:setTransType("TRANS_POP")
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransNextError.POP(token)\n")
    end
    fsm:setState(TransitionsMap.PopStart)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.TransNextError:JUMP (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransNextError.JUMP(token)\n")
    end
    fsm:clearState()
    ctxt:createGuard()
    ctxt:setTransType("TRANS_SET")
    ctxt:setEndState()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransNextError.JUMP(token)\n")
    end
    fsm:setState(TransitionsMap.JumpStart)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.TransNextError:WORD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransNextError.WORD(token)\n")
    end
    fsm:clearState()
    ctxt:createGuard()
    ctxt:setTransType("TRANS_SET")
    ctxt:setEndState(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransNextError.WORD(token)\n")
    end
    fsm:setState(TransitionsMap.SimpleTrans)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.TransGuard:SOURCE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::TransGuard.SOURCE(token)\n")
    end
    fsm:clearState()
    ctxt:createGuard(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::TransGuard.SOURCE(token)\n")
    end
...
function TransitionsMap.EndState:PUSH (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::EndState.PUSH(token)\n")
    end
    fsm:clearState()
    ctxt:setTransType("TRANS_PUSH")
    ctxt:setEndState()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::EndState.PUSH(token)\n")
    end
    fsm:setState(TransitionsMap.PushStart)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.EndState:POP (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::EndState.POP(token)\n")
    end
    fsm:clearState()
    ctxt:setTransType("TRANS_POP")
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::EndState.POP(token)\n")
    end
    fsm:setState(TransitionsMap.PopStart)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.EndState:JUMP (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::EndState.JUMP(token)\n")
    end
    fsm:clearState()
    ctxt:setTransType("TRANS_SET")
    ctxt:setEndState()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::EndState.JUMP(token)\n")
    end
    fsm:setState(TransitionsMap.JumpStart)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.EndState:WORD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::EndState.WORD(token)\n")
    end
    fsm:clearState()
    ctxt:setTransType("TRANS_SET")
    ctxt:setEndState(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::EndState.WORD(token)\n")
    end
    fsm:setState(TransitionsMap.SimpleTrans)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.EndStateError:LEFT_BRACE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::EndStateError.LEFT_BRACE(token)\n")
    end
    fsm:clearState()
    ctxt:createActionList()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::EndStateError.LEFT_BRACE(token)\n")
    end
    fsm:setState(TransitionsMap.ActionEnd)
    fsm:getState():Entry(fsm)
    fsm:pushState(ActionsMap.Start)
...
function TransitionsMap.SimpleTrans:SLASH (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::SimpleTrans.SLASH(token)\n")
    end
    fsm:clearState()
    ctxt:setTransType("TRANS_PUSH")
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::SimpleTrans.SLASH(token)\n")
    end
...
function TransitionsMap.SimpleTrans:LEFT_BRACE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::SimpleTrans.LEFT_BRACE(token)\n")
    end
    fsm:clearState()
    ctxt:createActionList()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::SimpleTrans.LEFT_BRACE(token)\n")
    end
    fsm:setState(TransitionsMap.ActionEnd)
    fsm:getState():Entry(fsm)
    fsm:pushState(ActionsMap.Start)
...
function TransitionsMap.PushTransition:PUSH (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::PushTransition.PUSH(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::PushTransition.PUSH(token)\n")
    end
    fsm:setState(TransitionsMap.PushStart)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.PushStart:LEFT_PAREN (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::PushStart.LEFT_PAREN(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::PushStart.LEFT_PAREN(token)\n")
    end
...
function TransitionsMap.PushError:RIGHT_PAREN (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::PushError.RIGHT_PAREN(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::PushError.RIGHT_PAREN(token)\n")
    end
...
function TransitionsMap.PushError:LEFT_BRACE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::PushError.LEFT_BRACE(token)\n")
    end
    fsm:clearState()
    ctxt:createActionList()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::PushError.LEFT_BRACE(token)\n")
    end
    fsm:setState(TransitionsMap.ActionEnd)
    fsm:getState():Entry(fsm)
    fsm:pushState(ActionsMap.Start)
...
function TransitionsMap.PushMap:WORD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::PushMap.WORD(token)\n")
    end
    fsm:clearState()
    ctxt:setPushState(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::PushMap.WORD(token)\n")
    end
...
function TransitionsMap.PushEnd:RIGHT_PAREN (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::PushEnd.RIGHT_PAREN(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::PushEnd.RIGHT_PAREN(token)\n")
    end
    fsm:setState(TransitionsMap.ActionStart)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.JumpStart:LEFT_PAREN (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::JumpStart.LEFT_PAREN(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::JumpStart.LEFT_PAREN(token)\n")
    end
...
function TransitionsMap.JumpError:RIGHT_PAREN (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::JumpError.RIGHT_PAREN(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::JumpError.RIGHT_PAREN(token)\n")
    end
    fsm:setState(TransitionsMap.ActionStart)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.JumpError:LEFT_BRACE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::JumpError.LEFT_BRACE(token)\n")
    end
    fsm:clearState()
    ctxt:createActionList()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::JumpError.LEFT_BRACE(token)\n")
    end
    fsm:setState(TransitionsMap.ActionEnd)
    fsm:getState():Entry(fsm)
    fsm:pushState(ActionsMap.Start)
...
function TransitionsMap.JumpMap:WORD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::JumpMap.WORD(token)\n")
    end
    fsm:clearState()
    ctxt:setEndState(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::JumpMap.WORD(token)\n")
    end
...
function TransitionsMap.JumpEnd:RIGHT_PAREN (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::JumpEnd.RIGHT_PAREN(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::JumpEnd.RIGHT_PAREN(token)\n")
    end
    fsm:setState(TransitionsMap.ActionStart)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.PopStart:LEFT_PAREN (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::PopStart.LEFT_PAREN(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::PopStart.LEFT_PAREN(token)\n")
    end
...
function TransitionsMap.PopStart:LEFT_BRACE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::PopStart.LEFT_BRACE(token)\n")
    end
    fsm:clearState()
    ctxt:createActionList()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::PopStart.LEFT_BRACE(token)\n")
    end
    fsm:setState(TransitionsMap.ActionEnd)
    fsm:getState():Entry(fsm)
    fsm:pushState(ActionsMap.Start)
...
function TransitionsMap.PopError:RIGHT_PAREN (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::PopError.RIGHT_PAREN(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::PopError.RIGHT_PAREN(token)\n")
    end
    fsm:setState(TransitionsMap.ActionStart)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.PopError:LEFT_BRACE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::PopError.LEFT_BRACE(token)\n")
    end
    fsm:clearState()
    ctxt:createActionList()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::PopError.LEFT_BRACE(token)\n")
    end
    fsm:setState(TransitionsMap.ActionEnd)
    fsm:getState():Entry(fsm)
    fsm:pushState(ActionsMap.Start)
...
function TransitionsMap.PopAction:RIGHT_PAREN (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::PopAction.RIGHT_PAREN(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::PopAction.RIGHT_PAREN(token)\n")
    end
    fsm:setState(TransitionsMap.ActionStart)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.PopAction:WORD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::PopAction.WORD(token)\n")
    end
    fsm:clearState()
    ctxt:setEndState(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::PopAction.WORD(token)\n")
    end
...
function TransitionsMap.PopArgs:RIGHT_PAREN (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::PopArgs.RIGHT_PAREN(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::PopArgs.RIGHT_PAREN(token)\n")
    end
    fsm:setState(TransitionsMap.ActionStart)
    fsm:getState():Entry(fsm)
end
...
function TransitionsMap.PopArgs:COMMA (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::PopArgs.COMMA(token)\n")
    end
    fsm:clearState()
    ctxt:createArgList()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::PopArgs.COMMA(token)\n")
    end
...
function TransitionsMap.ActionStart:LEFT_BRACE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::ActionStart.LEFT_BRACE(token)\n")
    end
    fsm:clearState()
    ctxt:createActionList()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::ActionStart.LEFT_BRACE(token)\n")
    end
    fsm:setState(TransitionsMap.ActionEnd)
    fsm:getState():Entry(fsm)
    fsm:pushState(ActionsMap.Start)
...
function TransitionsMap.ActionStartError:LEFT_BRACE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: TransitionsMap::ActionStartError.LEFT_BRACE(token)\n")
    end
    fsm:clearState()
    ctxt:createActionList()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : TransitionsMap::ActionStartError.LEFT_BRACE(token)\n")
    end
    fsm:setState(TransitionsMap.ActionEnd)
    fsm:getState():Entry(fsm)
    fsm:pushState(ActionsMap.Start)
...
function ParamsMap.Start:WORD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: ParamsMap::Start.WORD(token)\n")
    end
    fsm:clearState()
    ctxt:createParameter(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : ParamsMap::Start.WORD(token)\n")
    end
...
function ParamsMap.Start:RIGHT_PAREN (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: ParamsMap::Start.RIGHT_PAREN(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : ParamsMap::Start.RIGHT_PAREN(token)\n")
    end
    fsm:popState()
    fsm:paramsDone()
end
...
function ParamsMap.Start:DOLLAR (fsm, token)
    ...
    if ctxt.targetLanguage == 'PERL'
        ...
        fsm:getState():Exit(fsm)
        if fsm:getDebugFlag() then
            fsm:getDebugStream():write("ENTER TRANSITION: ParamsMap::Start.DOLLAR(token)\n")
        end
        -- No actions.
        if fsm:getDebugFlag() then
            fsm:getDebugStream():write("EXIT TRANSITION : ParamsMap::Start.DOLLAR(token)\n")
        end
...
function ParamsMap.Dollar:WORD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: ParamsMap::Dollar.WORD(token)\n")
    end
    fsm:clearState()
    ctxt:createParameter(token, "$")
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : ParamsMap::Dollar.WORD(token)\n")
    end
    fsm:setState(ParamsMap.ParamSeparator)
    fsm:getState():Entry(fsm)
end
...
function ParamsMap.ParamSeparator:COLON (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: ParamsMap::ParamSeparator.COLON(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : ParamsMap::ParamSeparator.COLON(token)\n")
    end
...
function ParamsMap.ParamSeparator:COMMA (fsm, token)
    ...
    if ctxt.targetLanguage == 'TCL'
        ...
        fsm:getState():Exit(fsm)
        if fsm:getDebugFlag() then
            fsm:getDebugStream():write("ENTER TRANSITION: ParamsMap::ParamSeparator.COMMA(token)\n")
        end
        fsm:clearState()
        ctxt:addParameter()
        if fsm:getDebugFlag() then
            fsm:getDebugStream():write("EXIT TRANSITION : ParamsMap::ParamSeparator.COMMA(token)\n")
        end
...
function ParamsMap.ParamSeparator:RIGHT_PAREN (fsm, token)
    ...
    if ctxt.targetLanguage == 'TCL'
        ...
        fsm:getState():Exit(fsm)
        if fsm:getDebugFlag() then
            fsm:getDebugStream():write("ENTER TRANSITION: ParamsMap::ParamSeparator.RIGHT_PAREN(token)\n")
        end
        fsm:clearState()
        ctxt:addParameter()
        if fsm:getDebugFlag() then
            fsm:getDebugStream():write("EXIT TRANSITION : ParamsMap::ParamSeparator.RIGHT_PAREN(token)\n")
        end
        fsm:popState()
        fsm:paramsDone()
    else
...
function ParamsMap.ParamType:SOURCE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: ParamsMap::ParamType.SOURCE(token)\n")
    end
    fsm:clearState()
    ctxt:setParamType(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : ParamsMap::ParamType.SOURCE(token)\n")
    end
...
function ParamsMap.NextParam:COMMA (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: ParamsMap::NextParam.COMMA(token)\n")
    end
    fsm:clearState()
    ctxt:addParameter()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : ParamsMap::NextParam.COMMA(token)\n")
    end
    fsm:setState(ParamsMap.Start)
    fsm:getState():Entry(fsm)
end
...
function ParamsMap.NextParam:RIGHT_PAREN (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: ParamsMap::NextParam.RIGHT_PAREN(token)\n")
    end
    fsm:clearState()
    ctxt:addParameter()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : ParamsMap::NextParam.RIGHT_PAREN(token)\n")
    end
    fsm:popState()
    fsm:paramsDone()
end
...
function ParamsMap.Error:SOURCE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: ParamsMap::Error.SOURCE(token)\n")
    end
    fsm:clearState()
    ctxt:clearParameter()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : ParamsMap::Error.SOURCE(token)\n")
    end
    fsm:popState()
    fsm:paramsError()
end
...
function ActionsMap.Start:WORD (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: ActionsMap::Start.WORD(token)\n")
    end
    fsm:clearState()
    ctxt:createAction(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : ActionsMap::Start.WORD(token)\n")
    end
...
function ActionsMap.Start:RIGHT_BRACE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: ActionsMap::Start.RIGHT_BRACE(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : ActionsMap::Start.RIGHT_BRACE(token)\n")
    end
    fsm:popState()
    fsm:actionsDone()
end
...
function ActionsMap.Name:LEFT_PAREN (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: ActionsMap::Name.LEFT_PAREN(token)\n")
    end
    fsm:clearState()
    ctxt:createArgList()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : ActionsMap::Name.LEFT_PAREN(token)\n")
    end
...
function ActionsMap.Name:EQUAL (fsm, token)
    ...
    if ctxt.targetLanguage == 'C_SHARP'
        ...
        fsm:getState():Exit(fsm)
        if fsm:getDebugFlag() then
            fsm:getDebugStream():write("ENTER TRANSITION: ActionsMap::Name.EQUAL(token)\n")
        end
        fsm:clearState()
        ctxt:setProperty()
        ctxt:createArgList()
        if fsm:getDebugFlag() then
            fsm:getDebugStream():write("EXIT TRANSITION : ActionsMap::Name.EQUAL(token)\n")
        end
...
function ActionsMap.End:SEMICOLON (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: ActionsMap::End.SEMICOLON(token)\n")
    end
    fsm:clearState()
    ctxt:addAction()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : ActionsMap::End.SEMICOLON(token)\n")
    end
...
function ActionsMap.PropertyAssignment:SOURCE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: ActionsMap::PropertyAssignment.SOURCE(token)\n")
    end
    fsm:clearState()
    ...
    ctxt:addAction()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : ActionsMap::PropertyAssignment.SOURCE(token)\n")
    end
    fsm:setState(ActionsMap.Start)
    fsm:getState():Entry(fsm)
end
...
function ActionsMap.Error:RIGHT_BRACE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: ActionsMap::Error.RIGHT_BRACE(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : ActionsMap::Error.RIGHT_BRACE(token)\n")
    end
    fsm:popState()
    fsm:actionsError()
end
...
function ArgsMap.Start:SOURCE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: ArgsMap::Start.SOURCE(token)\n")
    end
    fsm:clearState()
    ctxt:createArgument(token)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : ArgsMap::Start.SOURCE(token)\n")
    end
...
function ArgsMap.NextArg:COMMA (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: ArgsMap::NextArg.COMMA(token)\n")
    end
    fsm:clearState()
    ctxt:addArgument()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : ArgsMap::NextArg.COMMA(token)\n")
    end
...
function ArgsMap.NextArg:RIGHT_PAREN (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: ArgsMap::NextArg.RIGHT_PAREN(token)\n")
    end
    fsm:clearState()
    ctxt:addArgument()
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : ArgsMap::NextArg.RIGHT_PAREN(token)\n")
    end
    fsm:popState()
    fsm:argsDone()
end
...
function ArgsMap.Error:RIGHT_BRACE (fsm, token)
    ...
    fsm:getState():Exit(fsm)
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("ENTER TRANSITION: ArgsMap::Error.RIGHT_BRACE(token)\n")
    end
    if fsm:getDebugFlag() then
        fsm:getDebugStream():write("EXIT TRANSITION : ArgsMap::Error.RIGHT_BRACE(token)\n")
    end
    fsm:popState()
    fsm:argsError()
end
