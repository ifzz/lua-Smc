...
--    stream:write "-- generated by smc (http://smc.sourceforge.net/)\n"
    ...
    stream:write "\n"
    stream:write "local error = error\n"
    if not self.noCatchFlag then
        stream:write "local pcall = pcall\n"
    end
    if self.debugLevel >= 0 then
        stream:write "local tostring = tostring\n"
    end
    stream:write "local strformat = require 'string'.format\n"
    stream:write "\n"
    stream:write "local statemap = require 'statemap'\n"
    if fsm.source then
        stream:write "\n"
    ...
    stream:write "    local msg = strformat(\"Undefined Transition\\nState: %s\\nTransition: %s\\n\",\n"
